<template>
  <div class="table-responsive jumbotron">
    <h1 style="text-align: center; margin-top: 0px; margin-bottom: 60px; font-size: 50px">
      {{ title }}
    </h1>
      ...
      <thead>
        ...
        <tr v-for="(item, index) in cart" :key="item.id">
          ...
          <td style="text-align: center; width: 20%">
            <div>
              <a class="btn btn-sm btn-success" v-on:click.prevent="addItem(item, index)"
                >^
              </a>
            </div>
        ...
        <tr>
          ...
          <th>
            <a class="btn btn-sm btn-success" v-on:click.prevent="removeAll()"
              >Remove All</a
            >
          </th>
        </tr>
    ...
    <div v-if="cart.length > 0">
      <a class="btn btn-primary btn-block" v-on:click.prevent="makeOrder()">Make Order</a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    makeOrder: async function () {
      await this.$confirm(
        "Are you sure that you want to make this order?",
        "",
        "warning"
      ).then(() => {
        this.$prompt("Additional comments:").then((text) => {
          ...
          axios
            ...
            .then((dad) => {
              ...
              if (this.cooks.length != 0) { // Se houverem cooks disponíveis
                this.order.prepared_by = this.cooks[0].id; // Atualizar na base de dados o valor de quem vai preparar a order

                var payload = { // Informação necessária para a notificação
                  user: { name: "Food@Home" },
                  message: "A new order has arrived!",
                  destination: "cook",
                  destinationUser: { id: this.cooks[0].id, name: this.cooks[0].name },
                };
                
                this.cooks[0].available_at = null; // O Cook fica disponivel neste momento
                axios.put("api/users/" + this.cooks[0].id, this.cooks[0]); // Essa informação é atualizada na base de dados
                this.$socket.emit("private_message", payload); // É enviada uma notificação ao user a dizer que recebeu uma nova order
                
                axios.post("api/order", this.order).then((response) => { // Adicionada a nova order na base de dados
                  this.makeOrderItems(response.data.id); // Vai buscar os items da order
                  this.removeAll(); // Remove tudo da dashboard/cart
                });
                alert("Order completed! Processing...");
              } else { // Se não houverem Cooks disponíveis, a order é guardada na base de dados, mas não é guardada com
                ...
                alert("No available cooks at the moment. Order is waiting...");
              }
            });
        });
      });
    },
    filterCooks: function () {
      for (var i = this.cooks.length - 1; i >= 0; i--) {
        if (this.cooks[i].type != "EC" ||(this.cooks[i].type == "EC" &&(this.cooks[i].available_at == null || this.cooks[i].blocked == 1))) {
    ...
    assignOrderToCook: function (orderId) {
      //get available cooks
      //check who has been available the most
      //assign the order
    },
  },
};
